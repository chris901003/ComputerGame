...
struct Node {
    ...
    // 創建Node的資訊
    void setNode(vector<vector<int>> board, int prev_space_x, int prev_space_y, int deep, bool is_from_root) {
        this->board = board;
        ...
        this->is_from_root = is_from_root;
    }
    ...
    vector<Node> fetch_next_step() {
        ...
        for (int idx = 0; idx < 4; idx++) {
            ...
            Node node = Node();
            node.setNode(board, space_x, space_y, deep + 1, is_from_root);
...
Node find_twin_board(Node node) {
    ...
    Node twin_node = Node();
    twin_node.setNode(node.board, -1, -1, 0, false);
    return twin_node;
}
...
void solve() {
    ...
    if (!goal_node.is_from_root) {
        cout << "No answer" << endl;
        cout << "=============== End ===============" << endl;
        return;
    }

    vector<pair<int,int>> space_moves = fetch_path_node(goal_node);

    cout << "Finish" << endl;
    cout << "Total nodes: " << record.size()  << endl;
    int steps = space_moves.size() - 1;
    cout << "Total steps: " << steps << endl;
    cout << "Path:" << endl;
    int cur_x = root_node.space_x;
    int cur_y = root_node.space_y;
    for (auto &space:space_moves) {
        swap(root_node.board[cur_x][cur_y], root_node.board[space.first][space.second]);
...
int main() {
    ...
    std::cout << "Excution time: " << ms_int.count() << "ms\n";
    return 0;
}
